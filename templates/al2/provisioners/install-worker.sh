...
for binary in ${BINARIES[*]}; do
  if [[ -n "$AWS_ACCESS_KEY_ID" ]]; then
    ...
    sudo /bin/aws s3 cp --region $BINARY_BUCKET_REGION $S3_PATH/$binary .
    sudo /bin/aws s3 cp --region $BINARY_BUCKET_REGION $S3_PATH/$binary.sha256 .
  else
...
if [ "$PULL_CNI_FROM_GITHUB" = "true" ]; then
  ...
  if [[ -n "$AWS_ACCESS_KEY_ID" ]]; then
    echo "AWS cli present - using it to copy binaries from s3."
    sudo /bin/aws s3 cp --region $BINARY_BUCKET_REGION $S3_PATH/${CNI_PLUGIN_FILENAME}.tgz .
    sudo /bin/aws s3 cp --region $BINARY_BUCKET_REGION $S3_PATH/${CNI_PLUGIN_FILENAME}.tgz.sha256 .
  else
...
if [[ -n "$AWS_ACCESS_KEY_ID" ]]; then
  ...
  sudo /bin/aws s3 cp --region $BINARY_BUCKET_REGION $S3_PATH/$ECR_CREDENTIAL_PROVIDER_BINARY .
else
...
if [[ "$CACHE_CONTAINER_IMAGES" == "true" ]] && ! [[ ${ISOLATED_REGIONS} =~ $BINARY_BUCKET_REGION ]]; then
  ...
  KUBE_PROXY_ADDON_VERSIONS=$(sudo /bin/aws eks describe-addon-versions --addon-name kube-proxy --kubernetes-version=${K8S_MINOR_VERSION})
  KUBE_PROXY_IMGS=()
  ...
  VPC_CNI_ADDON_VERSIONS=$(sudo /bin/aws eks describe-addon-versions --addon-name vpc-cni --kubernetes-version=${K8S_MINOR_VERSION})
  VPC_CNI_IMGS=()
  ...
  PULLED_IMGS=()
  REGIONS=$(sudo /bin/aws ec2 describe-regions --all-regions --output text --query 'Regions[].[RegionName]')
